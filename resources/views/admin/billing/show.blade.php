                            ...
                            <main>
                                <div class="row contacts">
                                    <div class="col invoice-to">
                                        ...
                                        <h2 class="to">{{ $bill->c_id ?? 'Customer Name' }}</h2>
                                        <div class="address">{{ $bill->adr ?? 'Customer Address' }}</div>
                                        <div class="address">{{ $bill->mob_no ?? 'Customer Address' }}</div>
                                        <div class="email"><a href="">{{ $bill->email ?? 'email@example.com' }}</a></div>
                                    </div>
                                    <div class="col invoice-details">
                                        <h1 class="invoice-id">Bill # {{ $bill->id }}</h1>
                                        <div class="date">Date of Billing: {{ \Carbon\Carbon::parse($bill->date)->format('d F Y') ?? 'N/A' }}</div>
                                        ...
                                        @if($billings->count() > 0)
                                            @foreach ($billings as $key => $billing)
                                                <tr>
                                                    ...
                                                    <td class="qty-left">{{ $billing->qty ?? 0 }}</td>
                                                    <td class="text-left">{{ $billing->p_id ?? 'N/A' }}</td>
                                                    <td class="unit">{{ $billing->rate ?? 0 }}</td>
                                                    <td class="total">{{ $billing->amt ?? 0 }}</td>
                                                </tr>
                                            @endforeach
                                        @else
                                            <tr>
                                    ...
                                    <tfoot>
                                        @if($billings->count() > 0)
                                            <tr>
                                                ...
                                                <td colspan="2">{{ $bill->bil_amt ?? 0 }}</td>
                                            </tr>
                                            <tr>
                                                <td colspan="2"></td>
                                                <td colspan="2">Pre. Amount</td>
                                                <td colspan="2">{{ $bill->p_amt ?? 0 }}</td>
                                            </tr>
                                            <tr>
                                                <td colspan="2"></td>
                                                <td colspan="2">Total Amount</td>
                                                <td colspan="2">{{ $bill->tot_amt ?? 0 }}</td>
                                            </tr>
                                            <tr>
                                                <td colspan="2"></td>
                                                <td colspan="2">Receive Amount</td>
                                                <td colspan="2">{{ $bill->r_amt ?? 0 }}</td>
                                            </tr>
                                            <tr>
                                                <td colspan="2"></td>
                                                <td colspan="2">Balance Amount</td>
                                                <td colspan="2">{{ $bill->bal_amt ?? 0 }}</td>
                                            </tr>
                                        @endif
                                    </tfoot>
                                </table>
